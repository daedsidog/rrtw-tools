...
int main(int argc, char **argv) {
  ...
  dcc_logmsg("Parsing {}...", sgr::file(edu_filename));
  ...
  if (flags.generate_export_units) {
    ...
    dcc_logmsg("Generating {}...", sgr::file(eu_filename));
    ...
    dcc_logmsg("Finished generating {}.", sgr::file(eu_filename));
    return 0;
  }

  dcc_logmsg("Parsing {}...", sgr::file(dmb_filename));

  unordered_map<string, battle_model> battle_models =
      parse_battle_models(fmt::format("{}/{}", root_dir, dmb_filename));

  dcc_logmsg("Loading {}...", sgr::file(eu_filename));

  string export_units;
  if (freadall(fmt::format("{}/{}", root_dir, eu_filename).c_str(),
               export_units) == -1) {
    dcc_logerr("Could not read {}: {}", sgr::file(eu_filename), errmsg());
    exit(-1);
  }

  dcc_logmsg("Loading {}...", sgr::file(en_strs_filename));

  string en_strings;
  if (freadall(fmt::format("{}/{}", root_dir, en_strs_filename).c_str(),
               en_strings) == -1) {
    dcc_logerr("Could not read {}: {}.", sgr::file(en_strs_filename), errmsg());
    exit(-1);
  }
